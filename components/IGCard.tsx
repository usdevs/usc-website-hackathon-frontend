...
const IGCard: React.FC<IGInfoProps> = ({ imageKey, ig_info }) => {
  ...
  const firstUserOnOrg: UserOnOrgWithUser | null = ig_info?.userOrg?.length > 0 ? ig_info?.userOrg[0] : null
  const igHead: string = firstUserOnOrg?.user?.name || 'No name'
  const slug = ig_info?.slug || DEFAULT_PNG_NAME
  const imageSrc = '/orgs/' + slug + '.png'
  const inviteLink = ig_info?.inviteLink || (firstUserOnOrg ? 'https://t.me/' + firstUserOnOrg?.user?.telegramUserName : '')
