...
export const BookingConfirmationPopup: React.FC<BookingConfirmationPopupProps> = ({
                                                                                    onClose, isOpen, bookingDataFromSelection, startDate, unsuccessfulFormSubmitString, setUnsuccessfulFormSubmitString
                                                                                  }) => {
  ...
  const handleSubmit = useCallback(async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..BjJEsTYtqafQkQukN5T8eSarYVej5IPC6dGHFCaZYPY'
      ...
      <ModalContent>
        ...
        <ModalBody>
          {unsuccessfulFormSubmitString === "" ? <></> :
            <Box>
              {"Errors in form submission: " + unsuccessfulFormSubmitString}
            </Box>}
          <form onSubmit={handleSubmit} style={{ backgroundColor: "white", padding: "1rem" }}>
            ...
            <FormControl>
              <FormLabel htmlFor="start" marginTop="0.5rem">Start Time</FormLabel>
              <Box>{format(bookingDataFromSelection?.start || new Date(), "p")}</Box>
            </FormControl>
            <FormControl>
              <FormLabel htmlFor="end" marginTop="0.5rem">End Time</FormLabel>
