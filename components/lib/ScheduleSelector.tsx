...
const BookedDateCell = styled.div<{
  ...
  width: 100%;
  height: ${(props) =>
    25 * props.numOfBookedSlots + props.columnGap * (props.numOfBookedSlots - 1)}px;
...
export default class ScheduleSelector extends React.Component<PropsType, StateType> {
    ...
    for (let d = 0; d < props.numDays; d += 1) {
      ...
      for (let h = props.minTime; h < props.maxTime; h += 1) {
        for (let c = 0; c < props.hourlyChunks; c += 1) {
          let temp: number = 0;
          const dateFnsRepresentation = addMinutes(
            addHours(addDays(startTime, d), h),
            c * minutesInChunk,
          );
          //todo careful of off-by-one, the slot is 29min long and not 30 min long
          const filteredArray = props.bookings.filter((booking) => {
            const bookingFromDate = subMinutes(
              fromUnixTime(booking.from / 1000),
              60 / props.hourlyChunks,
            );
            const bookingToDate = addMinutes(
              fromUnixTime(booking.to / 1000),
              60 / props.hourlyChunks,
            );
            ...
            if (
              isAfter(dateFnsRepresentation, bookingFromDate) &&
              isAfter(bookingToDate, dateFnsRepresentation)
            ) {
              temp =
                differenceInMinutes(bookingToDate, bookingFromDate) / (60 / props.hourlyChunks);
              return true;
            }
            temp = 0;
            return false;
  ...
  updateAvailabilityDraft(selectionEnd: Date | null, callback?: () => void) {
    ...
    this.props.setBookingDataFromSelection({
      ...this.props.bookingDataFromSelection,
  ...
  renderDateCellWrapper = (timeCellInfo: TimeCellInfo): JSX.Element => {
    ...
    const selected = Boolean(
      this.state.selectionDraft.find((a) => isSameMinute(a, timeCellInfo.date)),
    );
  ...
  renderTimeLabel = (time: Date): JSX.Element => {
    return this.props.isTimeLabelsDisplayed ? (
  ...
  renderVenueLabel = (venueName: string): JSX.Element => {
    return this.props.renderVenueLabel ? (
      this.props.renderVenueLabel(venueName)
    ) : (
      <DateLabel>{venueName}</DateLabel>
    );
  };
